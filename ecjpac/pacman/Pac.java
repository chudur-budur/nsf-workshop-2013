...
public class Pac extends Agent implements Steppable
{
	...
	/** 
	 * Resets a Pac, to avoid the "new" in the resetAgents() 
	 * procedure, from now on, "new" will be only in the constructors.
	 **/
	public void reset()
	{
		super.reset();		
		stopper = pacman.schedule.scheduleRepeating(this, 0, 1);
		isDead = false ;
		lastAction = NOTHING ;
	}
	...
	*/
	...
	{
		MutableDouble2D previousLocation = (MutableDouble2D)location.clone();
		
		// now do the policy step, depending on the implementations.
		policyStep.doPolicyStep();
			...
			if (obj instanceof Ghost && location.distanceSq(pacman.agents.getObjectLocation(obj)) <= 0.2) // within 0.4 roughly
			{
				...
				else // ouch
				{
					pacman.schedule.scheduleOnceIn(0.5, new Steppable()             // so it happens next.  Should be after resetLEvel(), so we do 0.5 rather than 0.25
					{
						public void step(SimState state)
						{
							//System.out.println("eaten to death");
							die();
						}
					});  // the ghosts move a bit more
				}
	...
	public void die()
	{
		                               ...
		                               new Steppable()
		{
			...
			public void step(SimState state)
			{
				...
				{
					lastAction = (lastAction + 1) % 4;    // spin around
				}
