...
public class PacMan extends SimState
{
	...
	protected boolean frightenGhosts;  // signal for the ghosts
	...
	public PacMan(long seed)
	{
		...
		// add the Pacs
		if (pacs.length > 1) // schedule pac 1 first so he appears on the bottom initially
			pacs[1] = new Pac(this,  1);
		pacs[0] = new Pac(this,  0);
	}

	/** 
	 * Creates a PacMan simulation with the given random number seed 
	 * and the Pacs to play with along with the initial actions. 
	 **/
	public PacMan(long seed, Pac[] _pacs, int[] _actions)
	{
		super(seed);
	...
	/** Set the Pacs with the Pacs and their initial behaviours **/
	public void setPacs(Pac[] _pacs, int[] _actions)
	{
		pacs = null ;
		pacs = _pacs ;
		actions = _actions ;
	...
	{
		int count = 0;
		for(int i = 0; i < pacs.length; i++) if (!pacs[i].isDead) count++;
		return count;
	}
			...
			if (pacs[i] != null)
			{
				if (best == null ||
				        (best.location.distanceSq(location) > pacs[i].location.distanceSq(location) && ((count=1)==1) ||
				         best.location.distanceSq(location) == pacs[i].location.distanceSq(location) && 
	...
	/** 
	 * The "actions[]" stores the keyboard input, and this function accesses it.
	 * --- need to move into some PolicyStep object ??
	 **/
